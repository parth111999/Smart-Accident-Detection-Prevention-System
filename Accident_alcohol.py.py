...
time.sleep(0.5)
lcd_byte(0x01,LCD_CMD) # 000001 Clear display
lcd_string("Smart Automobile System",LCD_LINE_1)
time.sleep(0.5)
while 1:       
	# Print out results
	...
	if(vibration_data == False):
		if(seat_belt_data == True):
			...
			if(alcohol_data == True):
				...
				lcd_byte(0x01,LCD_CMD) # 000001 Clear display
				lcd_string("Engine Initiated ",LCD_LINE_1)
				time.sleep(0.5)
				GPIO.output(Buzzer, False)
		...
		pio.uart.println("AT+CMGS=\"+919967322355\"\r")
		pio.uart.println("Emergency Accident Spotted ")    
